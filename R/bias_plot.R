...
bias_plot <- function(object) {
  ...
  graphics::title(main = "Bias", cex.main = 0.9)
  graphics::points(data_sub$y2_hat, data_sub$y1, col = "blue", pch = 19, cex = 0.5)
  
  ### Add the fitted line from model 2 and model 4,
  graphics::abline(a = 0, b = models[[2]]$coef, lwd = 2, lty = 1, col = "black")
  graphics::abline(models[[4]]$coef, lwd = 2, lty = 2, col = "blue")
  
  ### Add the subtitle
  graphics::mtext(subtitle, side = 3, cex = 0.8)
  ### Add the left y graphics::axis
  graphics::axis(2, col = "black", las = 1)
  graphics::mtext("y1 and y2", side = 2, line = 2)
  graphics::box(col = "black")
  
  ### Add second plot: bias graphics::axis
  graphics::par(new = TRUE)
  ## Plot the bias plot and put graphics::axis scale on right
  plot(data_new$y2_hat, data_new$bias, xlab = "", ylab = "", axes = FALSE,
       col = "red", type = "l", lty = 1, lwd = 2)
  graphics::abline(h = 0, col = "black", lwd = 1)
  ## Add the right y graphics::axis and label
  graphics::mtext("Bias", side = 4, col = "red", line = 2.5)
  graphics::axis(4, col = "red", col.axis = "red", las = 1)
  ### Draw the x graphics::axis and add the label
  graphics::axis(1)
  graphics::mtext("BLUP of x", side = 1, col = "black", line = 2)
  ### Add the legend
  lm_bias <- lm(data_new$bias ~ data_new$y2_hat)
  if (stats::coef(lm_bias)[2] < 0) {
    graphics::legend("top", legend = c("Reference method (y2)", "New method (y1)",
                             "Bias"),
