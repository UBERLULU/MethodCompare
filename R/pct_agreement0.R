...
pct_agreement0 <- function(object) {
  ...
  data_agg$pct_agreement <- 1 - (stats::qnorm(0.975) * sig_d + abs(data_agg$bias_y1)) /
  ...
  for (j in nb_ind) {
    ...
    pct_agreement_j <- 1 - (stats::qnorm(0.975) * sig_d_j + abs(bias_j)) /
      abs(blup_x_j)
  ...
  crit_value6 <- stats::quantile(unlist(sim_max_d), c(0.95))
  ...
  # 95% confidence bands
  graphics::points(data_agg$y2_hat, data_agg$pct_agreement_lo_fit, col = "blue",
         type = "l", lty = 2)
  graphics::points(data_agg$y2_hat, data_agg$pct_agreement_up_fit, col = "blue",
         type = "l", lty = 2)
